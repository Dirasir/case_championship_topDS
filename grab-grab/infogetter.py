...
#            sleep(0.2)
    ...
    def get_reviews(soup_content, driver):
        ...
        sleep(2)  # Увеличил паузу для полной загрузки вкладки
        ...
        while scroll_step < 300:
            try:
                driver.execute_script(
                    "var container = document.getElementsByClassName('scroll__container')[1];"
                ...
                current_reviews_count = len(temp_soup.find_all("div", {"class": "business-reviews-card-view__review"}))
                
                print(f"Step {scroll_step + 1}: Loaded {current_reviews_count}/{reviews_count} reviews")
                
                if current_reviews_count == prev_reviews_count:
                    no_change_count += 1
                    if no_change_count >= 5: 
                        print(f"No more reviews loading. Total loaded: {current_reviews_count}")
                        break
                else:
                    no_change_count = 0
                    
                prev_reviews_count = current_reviews_count
                scroll_step += 1

                if current_reviews_count >= reviews_count:
                    print(f"All reviews loaded! {current_reviews_count}/{reviews_count}")
                    sleep(1)  
                    break
        ...
        try:
            soup_content = BeautifulSoup(driver.page_source, "lxml")
            review_elements = soup_content.find_all("div", {"class": "business-reviews-card-view__review"})
            
            for data in review_elements:
                try:
                    author_elem = data.find("div", {"class": "business-review-view__author-name"})
                    if not author_elem:
                        continue
                        
                    author_span = author_elem.find("span", {"itemprop": "name"})
                    author = author_span.getText() if author_span else "Unknown"

                    rating_meta = data.find('meta', {'itemprop': 'ratingValue'})
                    rating = rating_meta['content'] if rating_meta else None

                    date_meta = data.find('meta', {'itemprop': 'datePublished'})
                    date_iso = date_meta['content'] if date_meta else None

                    text_elem = data.find("span", {"class": "spoiler-view__text-container"})
                    text = text_elem.getText() if text_elem else ""
            ...
            print(f"Successfully parsed {len(reviews)} reviews out of {reviews_count} total")
